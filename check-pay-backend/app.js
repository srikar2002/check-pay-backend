...
app.post("/consent/:mobileNumber", (req, res) => {
  ...
  if (token) 
    jwt.verify(token,config.JWT_secret, (err, decoded)=> {
    if (!err) res.send({ url:"https://easyans.github.io/CheckPay", token: token });
    })
  obj={
  ...
  .then(()=>{
        ...
        .then(resp=>{
          ...
          if(resp)
            res.send({ url:"https://easyans.github.io/CheckPay", token: tokenjwt }); 
          else
            res.send({ url:"https://easyans.github.io/CheckPay/login.html", token: null });
        return
        }).catch(()=>{
